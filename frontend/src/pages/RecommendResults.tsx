    ...
    <div>
      ...
      <div className="grid-cards">
        {results.map((product) => {
          ...
          return (
            <div key={product.id} className="card" style={{ display: 'flex', flexDirection: 'column', padding: '1.5rem', justifyContent: 'space-between', position: 'relative' }}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginBottom: '1rem', fontSize: '0.9rem', color: '#f59e0b', fontWeight: 'bold' }}>
                  {product.rating ? `★ ${product.rating.toFixed(1)}` : '★ Unrated'}
                </div>
              </div>
              ...
              <div>
                ...
                <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px', marginBottom: '1rem' }}>
                  <span style={{ fontSize: '1.5rem', fontWeight: 700, color: '#0f172a' }}>₹{product.price}</span>
                  {discountExists && (
                    <>
                      <span style={{ textDecoration: 'line-through', color: '#94a3b8', fontSize: '0.9rem' }}>₹{product.original_price}</span>
